...
public class PingPongBuffer
{
    ...
    public PingPongBuffer(Resolution resolution, int volumeDepth, RenderTextureFormat format)
    {
        for (int i = 0; i < 2; i++)
        {
            textures[i] = new RenderTexture(resolution.width, resolution.height, 0, format);
