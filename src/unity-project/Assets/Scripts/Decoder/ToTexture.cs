...
public class ToTexture : MonoBehaviour
{
    ...
    static string tstimg2 = tstpath2 + "desktop.jpg";
    ...
    static string tstbin = projectpath1 + "src\\decoder\\csharp-testing\\CSharpTesting\\bin\\x64\\Debug\\netcoreapp3.1\\tstbinary.bin";
    ...
    void Start()
    {
        original = rawtex = opencv = new Texture2D(2, 2);
        Debug.Log(tstimg);

        byte[] original_bin = File.ReadAllBytes(tstimg);
        byte[] opencv_bin = File.ReadAllBytes(tstbin);
        ...
        string original_hex = BitConverter.ToString(original_bin);
        string opencv_hex = BitConverter.ToString(opencv_bin);

        original.LoadImage(original_bin);
        Debug.Log(ImageConversion.LoadImage(original, original_bin));
        // logo.LoadImage(pngBytes);
        // opencv.LoadImage(opencv_bin);

        byte[] rawtex_bin = original.GetRawTextureData();
        string rawtex_hex = BitConverter.ToString(rawtex_bin);
        // rawtex.LoadImage(rawtex_bin);

        File.WriteAllText("original.txt", original_hex);
        File.WriteAllText("rawtex.txt", original_hex);
        File.WriteAllText("opencv.txt", opencv_hex);
    }
