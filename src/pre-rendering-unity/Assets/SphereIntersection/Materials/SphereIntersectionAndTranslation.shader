Shader "Hidden/SphereIntersectionAndTranslation"
{
    Properties
    {
    ...
    SubShader
    {
        ...
        Pass
        {
            ...
            fixed4 frag(v2f i) : SV_Target
            {
                ...
                // Tc to latlon and to radians, switching lat and lon
                float2 latlon = tc * PI;

                // 1. To point on sphere
                float3 P2 = latLonToVector(latlon, 1);

                // 2. Ray from P1 to P2 (skipped)
                // 3. Get intersection P3 and P4
                float t2 = raySphereIntersection(P1, P2, C, 1).y;
                float3 P4 = P1 + float3(P2.x * t2, P2.y * t2, P2.z * t2);

                // 4. Get latlon2
                float2 latlon2 = vectorToLatLon(P4, 1) / PI;

                fixed4 col = tex2D(_MainTex, latlon2);

                
                float4 conv = float4(tc - latlon2, 0, 1);
