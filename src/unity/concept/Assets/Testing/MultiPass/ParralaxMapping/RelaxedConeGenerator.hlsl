float4 depth2relaxedcone(in float2 TexCoord: TEXCOORD0, in Sampler2D ReliefSampler, in float3 Offset)
    : COLOR
{
    ...
    float src_texel_depth = tex2D(ReliefSampler, TexCoord).w;                                                                // Original texel depth
    float cone_ratio = (ray_pos.z >= src_texel_depth) ? 1.0 : length(ray_pos.xy - TexCoord) / (src_texel_depth - ray_pos.z); // Compute the cone ratio
    float best_ratio = tex2D(ResultSampler, TexCoord).x;                                                                     // Check for minimum value with previous pass result
